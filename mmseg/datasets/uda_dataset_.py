    ...
    def __init__(self, source, target, cfg):
        ...
        if self.rcs_enabled:
            ...
            with open(
                ...
                # samples_with_class_and_n_reduced= {}
                # for c in range(len(self.CLASSES)):
                #     samples_with_class_and_n_reduced[str(c)] = []
                #     for ind_image in range(len(samples_with_class_and_n[str(c)])):
                #         image_name = str(samples_with_class_and_n[str(c)][ind_image])[26:31]
                #         matching = [s for s in self.source.img_infos if image_name in s['filename']]
                #         if len(matching)!=0:
                #              samples_with_class_and_n_reduced[str(c)].append(samples_with_class_and_n[str(c)][ind_image])
                # samples_with_class_and_n = samples_with_class_and_n_reduced
            # print('self.rcs_classes', self.rcs_classes)
    ...
    def get_rare_class_sample(self):
        ...
        return {
            **s1, 'target_img_metas': s2['img_metas'],
            'target_img': s2['img'], 'target_gt_semantic_seg': s2.get('gt_semantic_seg', -1)
        }
